var AppProcess = (function () {
  ...
  async function videoProcess(newVideoState) {
    if (newVideoState == video_states.None) {
      ...
      // serverProcess(
      // 	JSON.stringify({ Video_switch_off: "Video_switch_off", }),
      // 	rtp_vid_senders
      // )
      return
    }
    ...
    connection.ontrack = function (event) {
      ...
      if (event.track.kind == "video") {
        ...
        remote_aud_stream[connId]
          .getAudioTracks()
          .forEach((audTrack) => {
            remote_aud_stream[connId].removeTrack(audTrack)
          })
    ...
    socket.on("inform_me_about_other_user", function (users) {
      if (users) {
        for (var i = 0; i < users.length; i++) {
          addUser(users[i].user_id, users[i].connectionId)
          AppProcess.setNewConnection(users[i].connectionId)
        }
      }
    })
